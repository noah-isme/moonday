...
const feedbackSchema = z.object({
	...
	feedbackType: z.enum(RESPONSE_FEEDBACK_TYPES)
});
...
export const POST: RequestHandler = async ({ request }) => {
	try {
		...
		if (error instanceof z.ZodError)
			return json(
				{ error: { code: 'VALIDATION_ERROR', message: 'Choose a valid feedback option.' } },
				{ status: 400 }
			);
		return json(
			{ error: { code: 'DATABASE_ERROR', message: 'Unable to save feedback.' } },
			{ status: 500 }
		);
	}
};
...
export const GET: RequestHandler = async () => {
	try {
		...
		return json({
			totals,
			privacy:
				'Only category, provider, model, and response reference are stored. Response text is never copied into feedback.'
		});
	} catch {
		return json(
			{ error: { code: 'DATABASE_ERROR', message: 'Unable to load feedback totals.' } },
			{ status: 500 }
		);
	}
};
