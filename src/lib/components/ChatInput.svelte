...
>
	...
	<button
		...
		class="flex items-center justify-center w-10 h-10 rounded-full transition-all duration-300 relative group cursor-pointer border {chatStore.isWebSearchEnabled ? 'bg-violet-glow/20 border-violet-glow text-violet-glow shadow-[0_0_10px_rgba(139,92,246,0.5)] animate-pulse' : 'bg-transparent border-slate-gray/20 text-slate-gray hover:text-pale-silver hover:border-slate-gray/40'}"
	...
	>
		...
		<span class="absolute bottom-full mb-2 hidden group-hover:block bg-deep-navy border border-slate-gray/30 text-soft-white text-xs py-1 px-2 rounded whitespace-nowrap z-50 pointer-events-none transition-all duration-300">
